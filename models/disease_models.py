...
class DiseaseDetectionModel:
    ...
    def calculate_ensemble_weights(self, X_val, y_val):
        ...
        for name, model in self.models.items():
            try:
                ...
                print(f"Error calculating weights for {name}: {e}")
                continue
        
        # Get predictions from deep learning model
        if hasattr(self, 'deep_model') and self.deep_model is not None:
            try:
                deep_pred = self.deep_model.predict(X_val, verbose=0)
                ...
                print(f"Error calculating deep learning weights: {e}")
    ...
    def predict(self, X, use_ensemble=True):
        ...
        if use_ensemble and self.ensemble_weights:
            ...
            if hasattr(self, 'deep_model') and self.deep_model is not None:
                try:
                    deep_pred = self.deep_model.predict(X, verbose=0)
                    if len(deep_pred.shape) > 1:
                        deep_pred = deep_pred[:, 1]
                    predictions['deep_learning'] = deep_pred
                except Exception as e:
            ...
            if predictions:
                ...
                if total_weight > 0:
                    ...
                    final_pred = self.models['random_forest'].predict_proba(X)[:, 1]
                
                return final_pred
            else:
                # Fallback to random forest
                return self.models['random_forest'].predict_proba(X)[:, 1]
        else:
            # Use best individual model
            if self.ensemble_weights:
                best_model_name = max(self.ensemble_weights, key=self.ensemble_weights.get)
                if best_model_name == 'deep_learning' and hasattr(self, 'deep_model') and self.deep_model is not None:
                    pred = self.deep_model.predict(X, verbose=0)
                    if len(pred.shape) > 1:
                        pred = pred[:, 1]
                    return pred
