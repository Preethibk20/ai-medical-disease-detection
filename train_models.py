...
def evaluate_models(disease_detector, training_data):
    ...
    for disease, data in training_data.items():
        ...
        try:
            ...
            # Calculate AUC (handle case where all labels are same class)
            try:
                auc = roc_auc_score(y, predictions)
            except ValueError:
                auc = 0.5  # Default AUC when all labels are same class
            
            # Classification report
            try:
                report = classification_report(y, y_pred_binary, output_dict=True, zero_division=0)
            ...
            # Create dummy results for failed evaluations
            results[disease] = {
                'accuracy': 0.0,
                'auc': 0.5,
                'precision': 0.0,
                ...
                'predictions': np.zeros(len(data['labels'])),
